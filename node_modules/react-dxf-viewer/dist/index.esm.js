...
const DXFViewer = ({ file, className, onLoad, onError, cameraPosition, backgroundColor, lineColor, orbitControls, }) => {
    ...
    useEffect(() => {
        ...
        const camera = new THREE.OrthographicCamera(container.clientWidth / -2, container.clientWidth / 2, container.clientHeight / 2, container.clientHeight / -2, 0.1, 1000);
                ...
                if (parsed.entities) {
                    ...
                    parsed.entities.forEach((ent) => {
                        if (ent.type === 'LINE' && ent.vertices) {
                            const start = new THREE.Vector3(ent.vertices[0].x, ent.vertices[0].y, ent.vertices[0].z ?? 0);
                            const end = new THREE.Vector3(ent.vertices[1].x, ent.vertices[1].y, ent.vertices[1].z ?? 0);
                            ...
                            const arcCurve = new THREE.ArcCurve(ent.center.x, // cx
                            ent.center.y, // cy
                            ent.radius, // radius
                            ...
                            const pts3D = pts2D.map((p) => new THREE.Vector3(p.x, p.y, ent.center.z ?? 0));
                            ...
                            const center = new THREE.Vector3(ent.center.x, ent.center.y, ent.center.z ?? 0);
                            const radius = ent.radius;
                            const segments = 64;
                            const points = [];
                            for (let i = 0; i <= segments; i++) {
                                const theta = (i / segments) * Math.PI * 2;
                                points.push(new THREE.Vector3(center.x + Math.cos(theta) * radius, center.y + Math.sin(theta) * radius, center.z));
                            }
                            ...
                            const ctrlPts = ent.controlPoints.map((v) => new THREE.Vector3(v.x, v.y, v.z ?? 0));
                            ...
                            const pts = ent.vertices.map((v) => new THREE.Vector3(v.x, v.y, v.z ?? 0));
...
export { DXFViewer, DXFViewer as default };
