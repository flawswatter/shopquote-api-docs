 ...
 */
const DXFViewer = ({ file, className, onLoad, onError, cameraPosition, backgroundColor, lineColor, orbitControls, }) => {
    ...
    const handleResize = React.useCallback(() => {
        ...
        if (!container || !camera || !renderer || !scene)
            return;
    ...
    React.useEffect(() => {
        ...
        const camera = new THREE__namespace.OrthographicCamera(container.clientWidth / -2, container.clientWidth / 2, container.clientHeight / 2, container.clientHeight / -2, 0.1, 1000);
        camera.position.set(cameraPosition?.x ?? 0, cameraPosition?.y ?? 0, cameraPosition?.z ?? 5);
        ...
        const loadData = (text) => {
            try {
                ...
                if (parsed.entities) {
                    ...
                    const meshMaterial = new THREE__namespace.MeshBasicMaterial({
                        color: 0xcccccc,
                        side: THREE__namespace.DoubleSide,
                    });
                    const dashedLineMaterial = new THREE__namespace.LineDashedMaterial({
                    ...
                    parsed.entities.forEach((ent) => {
                        if (ent.type === 'LINE' && ent.vertices) {
                            const start = new THREE__namespace.Vector3(ent.vertices[0].x, ent.vertices[0].y, ent.vertices[0].z ?? 0);
                            const end = new THREE__namespace.Vector3(ent.vertices[1].x, ent.vertices[1].y, ent.vertices[1].z ?? 0);
                            const geometry = new THREE__namespace.BufferGeometry().setFromPoints([
                                start,
                                end,
                            ]);
                            if (ent.lineType?.toLocaleLowerCase() === 'dashed') {
                                const line = new THREE__namespace.Line(geometry, dashedLineMaterial);
                                line.computeLineDistances();
                                scene.add(line);
                            }
                            else {
                                const line = new THREE__namespace.Line(geometry, lineMaterial);
                                scene.add(line);
                            }
                            ...
                            const arcCurve = new THREE__namespace.ArcCurve(ent.center.x, // cx
                            ent.center.y, // cy
                            ent.radius, // radius
                            ...
                            const pts3D = pts2D.map((p) => new THREE__namespace.Vector3(p.x, p.y, ent.center.z ?? 0));
                            // 4) Build BufferGeometry and compute dash distances
                            const geometry = new THREE__namespace.BufferGeometry().setFromPoints(pts3D);
                            geometry.computeBoundingSphere(); // helps with frustum-culling
                            // 5) Create the line (uses your dashedLineMaterial)
                            const arcLine = new THREE__namespace.Line(geometry, lineMaterial);
                            // 6) Add to scene and expand your Box3
                            scene.add(arcLine);
                            pts3D.forEach((p) => box.expandByPoint(p));
                            hasGeometry = true;
                        }
                        else if (ent.type === 'CIRCLE') {
                            // 1. Compute points around the full circle
                            const center = new THREE__namespace.Vector3(ent.center.x, ent.center.y, ent.center.z ?? 0);
                            const radius = ent.radius;
                            const segments = 64;
                            const points = [];
                            for (let i = 0; i <= segments; i++) {
                                const theta = (i / segments) * Math.PI * 2;
                                points.push(new THREE__namespace.Vector3(center.x + Math.cos(theta) * radius, center.y + Math.sin(theta) * radius, center.z));
                            }
                            // 2. Use a LineLoop for closed shape
                            const geom = new THREE__namespace.BufferGeometry().setFromPoints(points);
                            const circle = new THREE__namespace.LineLoop(geom, lineMaterial);
                            scene.add(circle);
                            ...
                            const ctrlPts = ent.controlPoints.map((v) => new THREE__namespace.Vector3(v.x, v.y, v.z ?? 0));
                            ...
                            const curve = new THREE__namespace.CatmullRomCurve3(ctrlPts, false, 'centripetal', 0.5);
                            // 3) Sample it finely—128 segments for super smooth result
                            const samples = 128;
                            const pts = curve.getPoints(samples);
                            // 4) Build the geometry and dash distances
                            const geom = new THREE__namespace.BufferGeometry().setFromPoints(pts);
                            geom.computeBoundingSphere();
                            // 5) Create and add the dashed line
                            const splineLine = new THREE__namespace.Line(geom, lineMaterial);
                            ...
                            const pts = ent.vertices.map((v) => new THREE__namespace.Vector3(v.x, v.y, v.z ?? 0));
                            ...
                            if (ent.lineType?.toLowerCase() === 'dashed') {
                                ...
                                scene.add(new THREE__namespace.Line(geom, dashedLineMaterial));
                            }
                            else {
                                scene.add(new THREE__namespace.Line(geom, lineMaterial));
                            }
                            pts.forEach((p) => box.expandByPoint(p));
                            hasGeometry = true;
                        }
    ...
    return (React.createElement("div", { ref: containerRef, className: className, style: { width: '100%', height: '100%' } }));
};

exports.DXFViewer = DXFViewer;
exports.default = DXFViewer;
